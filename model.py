...
class Model(object):
    ...
    def __init__(self, config):
        # 在init中定义整个神经网络的训练模块，包含神经网络的搭建和优化器的选择。
    ...
    def embedding_layer(self, char_inputs, seg_inputs, config, name=None):
        ...
        with tf.variable_scope("char_embedding" if not name else name), tf.device('/GPU:0'):
            # 这里使用一个随机的数组生成字符串，表示字的embedding，简化代码
            self.char_lookup = tf.get_variable(
                name="char_embedding",
                shape=[self.num_chars, self.char_dim],
                initializer=self.initializer)
            # 输入char_inputs='常' 对应的字典的索引/编号/value为：8
            # self.char_lookup=[X(每句话的长度)*100]的向量，char_inputs字对应在字典的索引/编号/key=[1]
            embedding.append(tf.nn.embedding_lookup(self.char_lookup, char_inputs))
            # self.embedding1.append(tf.nn.embedding_lookup(self.char_lookup, char_inputs))
            # 这里的char_inputs是输入的语句，利用self.char_inputs = tf.placeholder获得

            # 这里的数据是将seg_inputs做embedding
            if config["seg_dim"]:
    ...
    def IDCNN_layer(self, model_inputs,
        ...
        with tf.variable_scope("idcnn" if not name else name):
            ...
            # 这里的self.layerInput_test也是为了sees.run后打印出来
    ...
    def create_feed_dict(self, is_train, batch):
        ...
        """
        # 在这里分别出去来batch的四个向量赋值给self.char_inputs, self.seg_inputs, self.targets, self.dropout
    ...
    def run_step(self, sess, is_train, batch):
        ...
        if is_train:
            # 此处之所以设置如此多的参数进入sess.run中是因为要得到其中的参数返回值，而真正需要feed dic的只有一个feed_dict这个数据是经过create_feed_dict修改的输入数据
            # 其格式是：chars, segs, tags 在model中对应的数据是 self.char_inputs   self.seg_inputs  self.targets  self.dropout
            ...
            model_inputs_test, layerInput_test, conv_test, w_test_1, w_test_2, char_inputs_test = sess.run(
                ...
                feed_dict)
            # 这里最重要的是在这里初始化model之后，所有的神经网络程序都会顺着mode的init来全部执行。在这里用于返回训练的loss返回值
